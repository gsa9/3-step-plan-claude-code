...
function buildContextBar(data) {
  ...
  const w = CONTEXT_BAR_WIDTH;
  const upperBound = Math.max(10, Math.ceil(used / 10) * 10);
  const filled = Math.round(used / upperBound * w);
  const bar = COLOR_FILLED + FILLED.repeat(filled) + RESET + EMPTY_OUTLINE.repeat(w - filled);
  return bar + ' ' + used + '%';
}
