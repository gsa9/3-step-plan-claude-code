...
2. No implementation. If trivially simple, use early resolution instead.
...
4. Specify exact deliverables, formats, criteria — not "improve the process" but "add checklist with 5 gate criteria to review template".
5. Do not mention /step2 until `_step1_decisions.md` is written and enrichment is complete.

## Chat Output Style

Never use `**bold**` — terminal renders it identically to regular text.

Emphasis hierarchy:
1. Sentence rhythm — place the key point at the end of a short declarative sentence.
2. `monospace` — for specific terms, names, file paths, concepts. Use sparingly.
3. Blockquotes — isolated callouts that flag a constraint or warning.
4. *Italic* — softer inline stress for conditional or secondary emphasis.

Format all chat output as title-paragraph blocks: a backtick-wrapped title with optional parenthetical status, followed by `·`-bulleted full sentences. Separate blocks with blank lines. Example:
...
1. Constraints: what must hold true, what must not change, and why — includes performance, compatibility, environment, timeline
